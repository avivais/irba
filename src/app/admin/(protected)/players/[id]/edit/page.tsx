        ...
        <div className="min-w-0">
          ...
          {playerPrecedence && (
            <p className="mt-0.5 text-sm tabular-nums text-zinc-500 dark:text-zinc-400">
              <span dir="ltr">
                מקום {playerRank} · ניקוד{" "}
                {formatScore(playerPrecedence.totalScore)}
              </span>
            </p>
          )}
        </div>
      ...
      <div className="mx-auto mt-6 flex w-full max-w-2xl md:max-w-4xl flex-col gap-6">
        ...
        <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
          ...
          {(() => {
            ...
            const dropInCount = playerCharges.filter(
              (c) => c.chargeType === "DROP_IN",
            ).length;
              ...
              <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  ...
                  <div className="flex items-center justify-between gap-2 py-2 text-sm">
                    ...
                    <span
                      className="flex-1 tabular-nums text-zinc-500 dark:text-zinc-400"
                      dir="ltr"
                    >
                    ...
                    <span
                      className="w-14 text-left tabular-nums font-medium text-zinc-800 dark:text-zinc-200"
                      dir="ltr"
                    >
                  ...
                  {!isDropIn && (
                    <div
                      className={`flex items-center justify-between gap-2 py-2 text-sm ${rankBreakdown.peerScore === null ? "opacity-40" : ""}`}
                    >
                      <span className="w-16 shrink-0 font-medium text-zinc-700 dark:text-zinc-300">
                        שחקנים
                      </span>
                      ...
                      >
                        {rankBreakdown.peerScore !== null
                          ? rankBreakdown.peerScore.toFixed(1)
                          : "—"}
                      </span>
                      {rankBreakdown.peerScore !== null ? (
                        <>
                  ...
                  {!isDropIn && (
                    <div
                      className={`flex items-center justify-between gap-2 py-2 text-sm ${rankBreakdown.winScore === null ? "opacity-40" : ""}`}
                    >
                      <span className="w-16 shrink-0 font-medium text-zinc-700 dark:text-zinc-300">
                        ניצחונות
                      </span>
                      ...
                      >
                        {rankBreakdown.winScore !== null
                          ? rankBreakdown.winScore.toFixed(1)
                          : "—"}
                      </span>
                      {rankBreakdown.winScore !== null ? (
                        <>
                          ...
                          >
                            ={" "}
                            {(
                              rankBreakdown.winScore *
                              rankBreakdown.weights.winWeight
                            ).toFixed(1)}
                          </span>
                        </>
                      ) : rankBreakdown.meetsThreshold === false &&
                        rankBreakdown.winThreshold > 0 ? (
                        <span className="w-32 text-left text-xs text-zinc-400">
                          חסרים{" "}
                          {rankBreakdown.winThreshold -
                            (rankBreakdown.matchStats?.total ?? 0)}{" "}
                          משחקים
                        </span>
                      ) : (
                        <span className="w-32 text-left text-xs text-zinc-400">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-green-700 dark:text-green-400">
                          {rankBreakdown.matchStats.wins} נצ׳
                        </span>
                        <span className="text-red-600 dark:text-red-400">
                          {rankBreakdown.matchStats.losses} הפ׳
                        </span>
                        {rankBreakdown.matchStats.ties > 0 && (
                          <span className="text-zinc-400">
                            {rankBreakdown.matchStats.ties} ת׳
                          </span>
                        )}
                        <span
                      ...
                      <div
                        className="h-2 w-20 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-700"
                        dir="ltr"
                      >
                        <div
                          className="h-full rounded-full bg-green-500"
                          style={{
                            width: `${(rankBreakdown.matchStats.winRatio * 100).toFixed(0)}%`,
                          }}
                        />
                      </div>
                      <span
