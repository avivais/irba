/**
 * Shared-expense engine — eligibility, share split.
 *
 * Concepts:
 * - A "shared expense" is a non-session charge (ball purchase, court repair…)
 *   split equally across players who meet an attendance threshold.
 * - Eligibility window is rolling: today − lookbackYears.
 * - Sessions counted: GameSession with date in window AND !isArchived AND isCharged.
 ...
 *
 * DB orchestration; the filtering/sorting logic is in computeEligible (pure).
 */
