"use client";

import { useActionState } from "react";
import { Loader2, Trash2 } from "lucide-react";
import {
  deleteSessionAction,
  type SessionActionState,
} from "@/app/admin/(protected)/sessions/actions";

type Props = {
  id: string;
  attendanceCount: number;
};

const initialState: SessionActionState = { ok: false };

export function SessionDeleteButton({ id, attendanceCount }: Props) {
  const boundAction = deleteSessionAction.bind(null, id);
  const [state, formAction, pending] = useActionState(boundAction, initialState);

  const canDelete = attendanceCount === 0;
  const title = canDelete
    ? "מחק מפגש"
    : `לא ניתן למחוק — יש ${attendanceCount} נרשמים`;

  function handleClick(e: React.MouseEvent<HTMLButtonElement>) {
    if (!canDelete) {
      e.preventDefault();
      return;
    }
    const confirmed = window.confirm(
      "האם למחוק מפגש זה? פעולה זו אינה ניתנת לביטול.",
    );
    if (!confirmed) {
      e.preventDefault();
    }
  }

  return (
    <div className="flex flex-col items-end gap-1">
      <form action={formAction}>
        <button
          type="submit"
          disabled={!canDelete || pending}
          title={title}
          onClick={handleClick}
          className="flex min-h-9 cursor-pointer items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 border-red-200 text-red-600 hover:bg-red-50 active:bg-red-100 focus:ring-red-300 disabled:border-zinc-200 disabled:text-zinc-400 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-950/30 dark:active:bg-red-900/50 dark:focus:ring-red-700 dark:disabled:border-zinc-700 dark:disabled:text-zinc-600"
        >
          {pending ? (
            <Loader2 className="h-4 w-4 animate-spin" aria-hidden />
          ) : (
            <Trash2 className="h-4 w-4" aria-hidden />
          )}
          מחק
        </button>
      </form>
      {!state.ok && state.message && (
        <p
          role="alert"
          className="max-w-[16rem] text-xs text-red-600 dark:text-red-400"
        >
          {state.message}
        </p>
      )}
    </div>
  );
}
