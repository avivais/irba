...
type Props = { playerId: string; year: number };

const initialState: PrecedenceActionState = { ok: false };

export function AggregateDeleteButton({ playerId, year }: Props) {
  const boundAction = deleteAggregateAction.bind(null, playerId, year);
  const [state, formAction, pending] = useActionState(boundAction, initialState);

  function handleClick(e: React.MouseEvent<HTMLButtonElement>) {
    const confirmed = window.confirm(
      `האם למחוק את נתוני שנת ${year}? פעולה זו אינה ניתנת לביטול.`,
    );
