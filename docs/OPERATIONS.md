...
2. If the session start time has passed (`date ≤ now`) → set `isClosed=true`, write `CLOSE_SESSION` audit (actor: cron), notify WA group
3. Run `checkLowAttendanceAlerts` — fire WA group message if any upcoming open session is below `session_min_players` and the alert hasn't fired for that session/tier
4. Returns `{ closed: string[], skipped: number, alerts: { earlyFired, criticalFired } }`
...
- **Assistant API / OpenClaw integration** — `POST /api/assistant/v1` is the typed production API used by Mikey/OpenClaw. Current production operations include `help`, `session_status`, `next_session`, admin-only `player_lookup`, admin `session_roster_add`, admin `session_roster_remove`, and self-service `player_register_add`, `player_register_cancel`, `player_register_status`. The low-level OpenClaw helper is `/root/.openclaw/workspace/bin/irba-assistant-api`. Natural WhatsApp roster/self-service commands are handled outside this repo by the local OpenClaw skill `/root/.openclaw/skills/irba-assistant/`, whose `scripts/irba_roster_command.py` performs parse → `player_lookup` → safe all-or-clarify → `session_roster_add/remove` for admin roster changes. The skill resolves phone/JID mentions directly and LID mentions only when OpenClaw/Baileys has a local reverse LID→phone mapping; unresolved LID-only mentions must block and ask for a name/phone. The skill must not partially mutate natural multi-player commands by default. IRBA owns configured notifications/cron behavior; OpenClaw owns user-facing conversation and should avoid duplicate broadcasts.
