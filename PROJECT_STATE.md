...
- **Match results panel** (`SessionMatchPanel`, `src/components/admin/session-match-panel.tsx`): last section on the session detail page (below attendance). Records multiple matches per session — admin picks team A / team B players from confirmed attendees, enters score. Full match list with edit and delete per row. Server actions: `createMatchAction`, `updateMatchAction`, `deleteMatchAction` in `src/app/admin/(protected)/sessions/[id]/matches/actions.ts`.
