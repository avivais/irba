...
- **`HourlyRate`**: `id`, `effectiveFrom DateTime @db.Date`, `pricePerHour Float`. Multiple rows; newest `effectiveFrom ≤ today` is the active rate. Full CRUD at `/admin/config/rates`.
...
#### Hourly rates (`/admin/config/rates`)

- List of all historical rates sorted newest-first; current rate (newest `effectiveFrom ≤ today`) highlighted in green with "תעריף נוכחי" badge.
- Add / edit / delete; duplicate-date guard per action.
- Config page (`/admin/config`) shows a quick-link card with the current rate at a glance.
- Components: `HourlyRateForm` (client, shared by new/edit), `HourlyRateDeleteButton` (client, `window.confirm`).
- Server actions in `src/app/admin/(protected)/config/rates/actions.ts`.
...
2. ✅ `HourlyRate` admin CRUD (`/admin/config/rates`) — done
...
*Last updated: Mar 2026 — Config system complete (AppConfig + config-keys.ts client/server split). HourlyRate model + full CRUD at /admin/config/rates. Session enhancements partially done: location/duration fields, config pre-fill, map links, overlap guard, admin auto-register. Remaining: session detail page, RSVP enforcement, Leaflet minimap.*
