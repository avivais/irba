...
- **Admin UI** (`/admin/config`): grouped settings form — מפגשים (day/time/duration/RSVP window), מיקום (name/lat/lng + live map links), חיוב (drop-in charge/debt threshold), שחקנים (default rank), משחקים (win score). Server action `updateConfigAction` upserts all keys in a transaction.
...
- **Docker**: `docker-compose.yml` with 3 services: `db` (Postgres 16-alpine), `app` (Next.js on `127.0.0.1:3004`), `wa` (Baileys/Express sidecar on internal port 3100). `Dockerfile` + `docker-entrypoint.sh` runs `prisma migrate deploy` then `next start`.
- **Deploy**: `./scripts/deploy.sh` — pre-deploy DB backup, then SSH to EC2: `git pull → docker compose build → docker compose up -d`. See `RUNBOOK.md` for full ops guide.
...
*Last updated: Mar 2026 — **Production deploy complete.** App live at `https://irba.sportgroup.cl`. All pre-production work done: waitlist promote action (`promoteWaitlistAction` with WA notification), WhatsApp Baileys sidecar (`wa/` service), auto-create cron endpoint (`GET /api/cron/auto-create`), schedule config keys in admin UI, CSP header, daily DB backup cron, ops runbook (`RUNBOOK.md`). Session enhancements complete: location/duration fields, config pre-fill, map links, OpenStreetMap minimap, overlap guard, admin auto-register, RSVP enforcement, unified session detail page, archive/date-filter, quick drop-in. Players + precedence unified. Next: WA QR scan to activate notifications, enable schedule in admin config, import player data, then user auth (step 3).*
