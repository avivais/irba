...
- **Date inputs in RTL context**: all `type="date"` and `type="datetime-local"` inputs carry `dir="ltr"` to prevent iOS Safari from scrambling the displayed date parts (e.g. "Apr 2026 13") when rendered inside an RTL page. Applies to: `session-form.tsx`, `challenge-form.tsx`, `hourly-rate-form.tsx`, `adjustment-form.tsx`, sessions filter, audit filter.
