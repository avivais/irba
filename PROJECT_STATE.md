...
- `<PeerRatingForm>` (`src/components/peer-rating-form.tsx`): sortable list using `@dnd-kit/core` + `@dnd-kit/sortable`; entire row is the drag target (not just the grip icon) — same pattern as reference repo; TouchSensor (delay:200, tolerance:5) + PointerSensor (distance:8) + KeyboardSensor; two full 44px ↑/↓ buttons per row with `onPointerDown stopPropagation` to prevent drag conflicts
