...
*Last updated: Mar 2026 — **Production live + UI polish complete.** App live at `https://irba.sportgroup.cl` with 20 registered players imported. UI polish: birthdate input now shows Israeli format (`dd.mm.yyyy`) with a `showPicker()` calendar button (works on desktop Chrome + mobile iOS); Add buttons across admin list pages replaced with compact circular `+` icons (fixes mobile header cramping); git commit hash baked at build time (`NEXT_PUBLIC_COMMIT_HASH`) and shown in admin footer + health endpoint. WA connected (`WA_NOTIFY_ENABLED=true`). Auto-create cron running hourly. Next: smoke-test end-to-end (create session → RSVP → verify), configure schedule in admin config, set up uptime monitoring, create admin player (Avi Vaisenberger, `isAdmin=true`), then user auth (step 3).*
