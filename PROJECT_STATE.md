...
- **Admin UI** (`/admin/config`): grouped settings form — מפגשים, לוח זמנים, מיקום, שחקנים, משחקים, תעריף שעתי (past rates collapsed), חיוב, וואטסאפ, **התראות נוכחות נמוכה**. Server action `updateConfigAction` upserts all keys in a transaction. The "התראות נוכחות נמוכה" section exposes the 7 `alert_*` keys: master toggle, early-alert card (enabled checkbox + hours-before input + template textarea), critical-alert card (same structure). Template variable hints shown inline: `{date}`, `{confirmed}`, `{min_players}`.
...
#### 8. Balanced team selection ✅ DONE
...
#### 9. Precedence-based waitlist (UI completion) ✅ DONE

All items shipped in earlier sessions: waitlist sorted by precedence score, manual promote button (“קדם”) on each waitlisted row, WA DM notification to promoted player.
