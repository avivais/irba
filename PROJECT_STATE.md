...
- **RSVP success banner**: auto-dismisses after 3 s. When user is added to the waiting list, shows amber “נוספת לרשימת ההמתנה — הרשמה תאושר אם יפנה מקום” and scrolls to `#waiting-list` section. `RsvpActionState` now includes `isWaitlisted?: boolean`.
...
- **Attendance sorting**: `src/lib/sort-attendances.ts` — REGISTERED players sorted by precedence score (desc) before DROP_IN players (FIFO by `createdAt`). Slice at `maxPlayers` determines confirmed vs waitlist.
