...
- OpenClaw ↔ IRBA assistant integration is active in production through Phase 3: read-only operations, admin roster mutations, admin-only `player_lookup`, and self-service RSVP (`player_register_add/cancel/status`) are deployed. Natural roster/self-service commands are orchestrated by the local OpenClaw skill `/root/.openclaw/skills/irba-assistant/`. Live QA covered status, duplicate self-register, self-cancel, and re-register on 2026-05-23. Remaining assistant work is tracked in [`docs/OPENCLAW_IRBA_INTEGRATION_PLAN.md`](./docs/OPENCLAW_IRBA_INTEGRATION_PLAN.md): production-group mention QA, UX/help polish, optional sensitive financial ops, hardening, and optional automation.
