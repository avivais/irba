...
- **Date inputs in RTL context**: all `type="date"` and `type="datetime-local"` inputs carry `dir="ltr"` to prevent iOS Safari from scrambling the displayed date parts (e.g. "Apr 2026 13") when rendered inside an RTL page. Applies to: `challenge-form.tsx`, `hourly-rate-form.tsx`, `adjustment-form.tsx`, sessions filter, audit filter.
...
- **Admin sessions list filter**: uses reusable `DateFieldIL` (`src/components/admin/date-field-il.tsx`) — `d.m.yyyy` text input + hidden `YYYY-MM-DD` form value + overlaid native `type="date"` picker. Same cross-platform fix as session form, applied to `from`/`to` filter fields.
